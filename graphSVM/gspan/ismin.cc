...
bool gSpan::is_min()
{
	...
	DFS_CODE.toGraph(GRAPH_IS_MIN); // 現在のDFSCODEのグラフをGRAPH_IS_MINに格納
	DFS_CODE_IS_MIN.clear();

	Projected_map3 root;
	EdgeList edges;

	for (unsigned int from = 0; from < GRAPH_IS_MIN.size(); ++from) // 全ての頂点
		if (get_forward_root(GRAPH_IS_MIN, GRAPH_IS_MIN[from], edges)) // edgesが空でない
